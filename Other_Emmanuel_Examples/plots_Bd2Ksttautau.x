...
events->Add("events_177750239.root");
...
TCanvas* chi2nnom = new TCanvas("chi2nnom","chi2nnom");
TH1F* hchi2nnom = new TH1F("hchi2nnom",";chi2nnom; a.u.",100,0.,10.);
gStyle->SetOptStat(1111110);
events->Draw("TaumVertex.chi2*3 >>hchi2nnom",cut);
TLatex tte;
TString textee="Bd #rightarrow K* #tau #tau, #tau^{#pm} #rightarrow 3p ";
tte.DrawLatexNDC(0.2,0.955,textee);
TString textee2="#tau decay vertex";
tte.DrawLatexNDC(0.2,0.9,textee2);
gPad -> SetLogy(1);
...
TCanvas* cc6 = new TCanvas("pull_TauVertex_x","pull_TauVertex_x");
TH1F*  px = new TH1F("px","; Pull x_{TaumVertex}; Events",100,-5,5);
events -> Draw( " (TaumVertex.position.x - TaumMCDecayVertex.x[0]) / TMath::Sqrt( TaumVertex.covMatrix[0] ) >> px",cut);
px->Fit("gaus");

TCanvas* cc7 = new TCanvas("pull_TauVertex_y","pull_TauVertex_y");
TH1F*  py = new TH1F("py","; Pull y_{TaumVertex}; Events",100,-5,5);
events -> Draw( "(TaumVertex.position.y - TaumMCDecayVertex.y[0]) / TMath::Sqrt( TaumVertex.covMatrix[2] ) >> py",cut);
py->Fit("gaus");

TCanvas* cc8 = new TCanvas("pull_TauVertex_z","pull_TauVertex_z");
TH1F*  pz = new TH1F("pz","; Pull z_{TaumVertex}; Events",100,-5,5);
events -> Draw( "(TaumVertex.position.z - TaumMCDecayVertex.z[0])  / TMath::Sqrt( TaumVertex.covMatrix[5] ) >> pz",cut);
pz->Fit("gaus");
// --------------------------------------------------------------------------------
